...
const DashboardLayout = () => {
  ...
  return (
    <div className={darkMode ? "dark" : ""} dir={isRTL ? "rtl" : "ltr"}>
      <div
        className={`flex min-h-screen transition-colors duration-300 ${darkMode ? "bg-[#111827]" : "bg-[#f0f4f8]"}`}
      >
        {/* Mobile Backdrop */}
        <AnimatePresence>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <main
            className={`flex-1 pt-24 pb-12 transition-all duration-300 
            ${
              isRTL
                ? isSidebarOpen
                  ? "lg:pr-[260px] pr-0"
        ...
        <AnimatePresence>
          {showSettings && (
            <>
              ...
              />
              <motion.div
                initial={{ x: isRTL ? -350 : 350 }}
                animate={{ x: 0 }}
                exit={{ x: isRTL ? -350 : 350 }}
                ...
                <section className="mb-8">
                  ...
                  <div className="flex flex-col gap-5">
                    <ThemeOption
                      ...
                      previewImg="/assets/light.png"
                    />
                    <ThemeOption
                      label="Dark"
                      isActive={darkMode}
                      onClick={() => setDarkMode(true)}
                      previewImg="/assets/dark.png"
                    />
                  </div>
                </section>
                ...
                <section className="mb-8">
                  ...
                  <div className="flex bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                    <button
                      ...
                      className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${sidebarTheme === "light" ? "bg-white text-black shadow-sm" : "text-gray-500"}`}
                    >
                      {sidebarTheme === "light" && <FiCheck />} Light
                    </button>
                    <button
                      onClick={() => setSidebarTheme("dark")}
                      className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${sidebarTheme === "dark" ? "bg-black text-white shadow-sm" : "text-gray-500"}`}
                    >
                      ...
                      >
                        {topbarColor === color.class && (
                          <div className="bg-white rounded-full p-0.5 shadow-sm">
                            <FiCheck className="text-blue-600" size={10} />
                          </div>
                        )}
                      </button>
                    ))}
                ...
                <section className="mb-8 pt-4 border-t dark:border-gray-700">
                  <div className="flex justify-between items-center">
                    ...
                    <div
                      ...
                      className={`w-11 h-6 rounded-full relative cursor-pointer transition-colors duration-200 ${isRTL ? "bg-blue-500" : "bg-gray-300"}`}
                    >
                      <motion.div
                        animate={{ x: isRTL ? (isRTL ? -22 : 22) : 0 }}
...
const ThemeOption = ({ label, isActive, onClick, previewImg }) => (
  <div onClick={onClick} className="group cursor-pointer">
    <div
      className={`relative rounded-xl overflow-hidden border-2 transition-all duration-300 ${isActive ? "border-blue-500 scale-[1.02] shadow-md" : "border-transparent opacity-70 hover:opacity-100 hover:border-gray-200"}`}
    >
      ...
      {isActive && (
        <div className="absolute inset-0 bg-blue-500/5 flex items-center justify-center">
          <div className="bg-blue-600 text-white rounded-full p-1 shadow-lg border-2 border-white translate-y-2">
            <FiCheck size={14} />
          </div>
        </div>
      )}
    ...
    <p
      className={`text-center mt-2 text-[11px] font-bold uppercase tracking-widest ${isActive ? "text-blue-500" : "text-gray-400"}`}
    >
      {label}
    </p>
  </div>
