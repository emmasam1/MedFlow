...
import {
    ...
    RiSearch2Line, RiStethoscopeLine, RiBuilding4Line,
    RiDragMove2Line, RiAlertFill, RiChatHistoryLine,
    RiShieldFlashLine, RiUserAddLine, RiAlarmWarningLine
} from "react-icons/ri";
                ...
                <>
                    ...
                    >
                        <div className="flex justify-between items-center mb-8">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900">Shift Handover</h2>
                        ...
                        <div className="space-y-6">
                            <div className="bg-blue-50 p-5 rounded-2xl border border-blue-100">
                                <p className="text-[10px] font-black uppercase text-blue-600 mb-1">Standard Protocol (SBAR)</p>
                                <p className="text-xs text-blue-800 leading-relaxed">
                                    Notes entered here are logged into the clinical audit trail and cannot be edited after sealing.
                                </p>
                            </div>

                            <div className="space-y-5">
                                {["Situation", "Background", "Assessment", "Recommendation"].map((field) => (
                                    <div key={field}>
                                        <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">{field}</label>
                                        <textarea
...
// --- HELPER COMPONENT: COVERAGE MONITOR ---
const CoverageMonitor = ({ shifts }) => {
    const departments = ["Emergency", "Cardiology", "Surgery", "ICU", "Pediatrics"];
    
    const getStats = (dept) => {
      // We only count staff who are NOT on-call as "Active" coverage
      const active = shifts.filter(s => s.department === dept && !s.isOnCall).length;
      const required = dept === "ICU" || dept === "Surgery" ? 5 : 2; 
      const ratio = (active / required) * 100;
      return { active, required, ratio };
    };
  
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
        {departments.map((dept) => {
          const { active, required, ratio } = getStats(dept);
          const isCritical = ratio < 100;
  
          return (
            <div key={dept} className="bg-white p-5 rounded-[2rem] border border-slate-100 shadow-sm relative overflow-hidden group">
               <div className="flex justify-between items-center mb-3">
                <h4 className="font-black text-slate-800 text-sm">{dept}</h4>
                <span className={`text-[10px] font-black px-2 py-1 rounded-lg uppercase ${
                  isCritical ? "bg-red-50 text-red-600" : "bg-emerald-50 text-emerald-600"
                }`}>
                  {isCritical ? "Understaffed" : "Optimal"}
                </span>
              </div>
  
              <div className="flex items-end gap-2 mb-3">
                <span className="text-2xl font-black text-slate-900">{active}</span>
                <span className="text-xs font-bold text-slate-400 mb-0.5">/ {required} Active</span>
              </div>
              ...
              <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                <motion.div
                  ...
                  className={`h-full rounded-full ${
                    ratio <= 50 ? "bg-red-500" : ratio < 100 ? "bg-amber-500" : "bg-emerald-500"
                  }`}
                />
              </div>
            </div>
...
const ShiftManagement = () => {
    ...
    const handleAssignSubmit = (e) => {
        e.preventDefault();
        if (!newRotation.staffId) return toast.error("Please select a professional");
    ...
    const handleDeleteShift = (id) => {
        if (window.confirm("Are you sure you want to remove this professional from the active roster?")) {
    ...
    return (
        <div className="max-w-[1600px] mx-auto px-6 py-10 min-h-screen bg-slate-50/30">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
                <div>
                    ...
                    <p className="text-slate-500 font-medium mt-1 italic">Enterprise Hospital Management System</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <StatCard title="Day Rotation" value={`${shifts.filter(s => s.shiftType === "Day").length} Staff`} color="orange" icon={<RiSunFoggyLine />} />
                <StatCard title="Night Rotation" value={`${shifts.filter(s => s.shiftType === "Night").length} Staff`} color="indigo" icon={<RiMoonClearLine />} />
                <StatCard title="Active Units" value="04 Units" color="emerald" icon={<RiBuilding4Line />} />
            </div>

            <CoverageMonitor shifts={shifts} />
            ...
            <div className="flex items-center gap-1 bg-slate-200/50 p-1.5 rounded-2xl w-fit mb-8">
                {tabs.map((tab) => (
                    <button
                        ...
                        className={`relative px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-colors flex items-center gap-2 ${currentTab === tab.id ? "text-blue-600" : "text-slate-500 hover:text-slate-700"
                            }`}
                    >
                        {tab.icon}
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 overflow-hidden">
                <div className="p-8 border-b border-slate-50 flex flex-col sm:flex-row justify-between items-center bg-slate-50/50 gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="font-black text-slate-800 uppercase tracking-widest text-xs">Live Facility Board</h3>
                    </div>
                ...
                <div className="overflow-x-auto p-6">
                    <AnimatePresence mode="wait">
                                                ...
                                                >
                                                    <td className="px-6 py-5 rounded-l-2xl">
                                                        <div className="flex items-center gap-4">
                                                            {isAdmin && <RiDragMove2Line className="text-slate-200 group-hover:text-blue-500 transition-colors" />}
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-[10px] uppercase font-black text-blue-500 tracking-wider">{staff.role}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    <td className="px-6 py-5">
                                                        <div className="flex items-center gap-3 bg-slate-50 w-fit px-4 py-2 rounded-xl border border-slate-100">
                                                            {staff.shiftType === "Day" ? <RiSunFoggyLine className="text-orange-500" /> : <RiMoonClearLine className="text-indigo-500" />}
                                                            <span className="text-xs font-black text-slate-700">{staff.startTime} - {staff.endTime}</span>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-5 text-center">
                                                        <button
                                                            onClick={() => { setSelectedStaff(staff); setIsHandoverOpen(true); }}
                                                    ...
                                                    <td className="px-6 py-5 rounded-r-2xl text-right">
                                                        <div className="flex items-center justify-end gap-1">
                                                            ...
                                                            >
                                                                <RiAlarmWarningLine
                                                                    size={15}
                                                                    className={staff.isOnCall ? "animate-pulse" : ""}
                                                                />
                                                            </button>
                                                            <button
                        ...
                        {currentTab === "oncall" && (
                            ...
                            >
                                <RiAlarmWarningLine size={40} className="mx-auto text-slate-300 mb-4" />
                                <h3 className="text-xl font-bold text-slate-800">Standby Monitoring</h3>
                                <p className="text-slate-500">Only professionals currently marked as "On-Call" will appear here.</p>
                            </motion.div>
                        )}
            ...
            {/* ASSIGNMENT MODAL */}
            <Modal isOpen={isAssignModalOpen} onClose={() => setIsAssignModalOpen(false)} title="New Rotation Assignment">
                <form onSubmit={handleAssignSubmit} className="space-y-6">
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">Department Unit</label>
                                <select
                            ...
                            <div>
                                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">Shift Type</label>
                                <div className="flex bg-slate-50 p-1.5 rounded-2xl">
                                    <button type="button" onClick={() => setNewRotation({ ...newRotation, shiftType: 'Day' })} className={`flex-1 py-2.5 rounded-xl font-black text-[10px] uppercase transition-all ${newRotation.shiftType === 'Day' ? 'bg-white shadow text-orange-600' : 'text-slate-400'}`}>Day</button>
                                    <button type="button" onClick={() => setNewRotation({ ...newRotation, shiftType: 'Night' })} className={`flex-1 py-2.5 rounded-xl font-black text-[10px] uppercase transition-all ${newRotation.shiftType === 'Night' ? 'bg-white shadow text-indigo-600' : 'text-slate-400'}`}>Night</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">Check-In</label>
                                <input type="time" className="w-full p-4 bg-slate-50 rounded-2xl border-none font-bold text-slate-700" />
                            </div>
                            <div>
                                <label className="text-[10px] font-black uppercase text-slate-400 mb-2 block tracking-widest">Check-Out</label>
                                <input type="time" className="w-full p-4 bg-slate-50 rounded-2xl border-none font-bold text-slate-700" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex gap-4 pt-6">
                        <button type="button" onClick={() => setIsAssignModalOpen(false)} className="flex-1 py-4 font-bold text-slate-400 hover:text-slate-600 transition-colors">Dismiss</button>
                        <button type="submit" className="flex-[2] py-4 bg-blue-600 text-white rounded-2xl font-bold shadow-xl shadow-blue-100 hover:bg-blue-700 transition-all">Confirm Assignment</button>
                    </div>
                </form>
            </Modal>
        </div>
