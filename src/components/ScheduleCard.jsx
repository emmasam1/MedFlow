 ...
 * ScheduleCard
 * Props: 
 *  - type: "doctor" | "specialist" (determines if showing queue or appointments)
 */
const ScheduleCard = ({ type }) => {
  ...
  const selectDay = (day) => {
    ...
    setSelectedDay(day);
    if (day >= startIndex + VISIBLE_DAYS) setStartIndex(Math.min(day - VISIBLE_DAYS + 1, days.length - VISIBLE_DAYS));
    if (day <= startIndex) setStartIndex(Math.max(day - 1, 0));
  };
  ...
  /* THEME */
  const containerBg = darkMode ? "bg-[#1a202c] border-gray-700" : "bg-white border-gray-100";
  ...
  const cardBg = darkMode ? "bg-gray-800 border-gray-700 hover:bg-gray-750" : "bg-white border-gray-100 hover:shadow-md hover:border-blue-100";
  const badgeBg = darkMode ? "bg-gray-700 text-gray-200" : "bg-gray-50 text-gray-600";

  return (
    <div className={`${containerBg} rounded-xl shadow-lg p-4 border flex flex-col h-[400px] transition-colors duration-300`}>
      {/* HEADER */}
      <div className="flex items-center justify-between mb-4 flex-shrink-0">
        <h2 className={`font-bold text-sm ${textPrimary}`}>
          {user?.role?.toLowerCase() === "doctor" ? "Queue" : "Appointments"}
        </h2>
        <div className="flex items-center gap-3 px-3 py-1 rounded-full">
          <button onClick={() => changeMonth(-1)} className={`${hoverBg} ${darkMode ? "text-white" : "text-gray-700"} p-1 rounded-full transition-colors duration-300 cursor-pointer`}>
          ...
          {days.slice(startIndex, startIndex + VISIBLE_DAYS).map((d) => (
            <button
              ...
              className={`relative flex flex-col items-center min-w-[50px] py-2 rounded-xl transition-all duration-300 ${
                selectedDay === d.date ? "text-blue-500" : `${textMuted} ${hoverBg}`
              }`}
            >
              ...
              {selectedDay === d.date && (
                <motion.div layoutId="activeDay" className={`absolute inset-0 rounded-xl -z-10 ${darkMode ? "bg-blue-900/30 border border-blue-700" : "bg-blue-50 border border-blue-200"}`} />
              )}
            </button>
          ))}
        </div>
          ...
          {todaysItems.length === 0 ? (
            ...
            todaysItems.map((item) => (
              <motion.div
                ...
                className={`group flex items-center justify-between border rounded-2xl p-2 transition-all ${cardBg}`}
              >
                <div className="flex gap-4">
                  <div className={`w-0.75 rounded-full ${type === "doctor" ? "bg-blue-500" : "bg-green-500"}`} />
                  <div>
                    <p className={`font-bold capitalize ${textPrimary}`}>{item.patientName}</p>
                    {/* {type === "specialist" && <p className="text-xs font-semibold text-blue-500 mb-1">{item.department}</p>} */}
                    {/* {type === "specialist" && <p className={`text-[11px] flex items-center gap-1 ${textMuted}`}>🩺 {item.assignedDoctor}</p>} */}
                  </div>
                </div>
                {/* <div className="text-right">
                  <span className={`text-xs font-bold px-2 py-1 rounded-md ${badgeBg}`}>
                    {type === "doctor" ? item.status : item.time}
                  </span>
                </div> */}
              </motion.div>
            ))
